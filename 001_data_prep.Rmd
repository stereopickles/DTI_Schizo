...
lst <- combined_behav %>% group_by(assessment_description) %>% slice(1) %>% select(assessment, assessment_description) %>% arrange(assessment_description)
...
fulldf <- fdf %>% 
  mutate(track_group = ifelse(track_group %in% c('Left', 'FATL', 'FAT_L', 'IFGL-SFGL', 'Aslant_L', 'Aslant_:'), 'FATL', ifelse(track_group %in% c('mid-coronal', 'Right', 'FATR', 'FAT_R', 'IFGR-SFGR', 'Aslant_R'), 'FATR', ifelse(track_group %in% c('UF_R', 'UF__R', 'UF-R'), 'UF_R', track_group)))) %>%
...
missing_FAT <- missing_tracts %>% filter(track_group %in% c("FATR", "FATL"))
missing_UF <- missing_tracts %>% filter(track_group %in% c("UFL", "UFR"))
